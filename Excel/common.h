...
public:
    ...
    FormulaError& operator=(FormulaError rhs);

private:
	Category category_;
    const std::string ref_= "#REF!";
    const std::string value_= "#VALUE!";
    const std::string div_= "#DIV/0!";
};
...
public:
    ...
    virtual void ValidCell(Position pos) const = 0;
    virtual bool CheckCell(Position pos) const = 0;
    // Очищает ячейку.
