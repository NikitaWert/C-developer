...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(std::function<double(Position)> fun) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function<double(Position)> fun) const override {
                using namespace std::literals;
                double result = 0;
                switch (type_) {
                case Type::Add:
                    result = lhs_->Evaluate(fun) + rhs_->Evaluate(fun);
                    break;
                case Type::Subtract:
                    result = lhs_->Evaluate(fun) - rhs_->Evaluate(fun);
                    break;
                case Type::Multiply:
                    ...
                    break;
                case Type::Divide:
                    if (!std::isfinite(lhs_->Evaluate(fun) / rhs_->Evaluate(fun)))
        ...
        public:
            ...
            double Evaluate(std::function<double(Position)> fun) const override {
                if (!fun) {
                    throw std::runtime_error("ERROR: func is empty");
                }
                return fun(*cell_);
            }
...
double FormulaAST::Execute(std::function<double(Position)> fun) const {
    return root_expr_->Evaluate(fun);
}
